...
class UIKitMovieList : ConstraintLayout {
    ...
    private val mAdapter by lazy { UIKitMovieItemAdapter() }
...
//        })
    ...
    fun addItem(item: MovieUiKitModel) {
    ...
    fun addDatas(datas: List<MovieUiKitModel>) {
        val itemsToAdd = mutableListOf<MovieUiKitModel>()
        for (item in datas) {
            val existingItemIndex = mAdapter.data.indexOfFirst { it.id == item.id }
            if (existingItemIndex != -1) {
                mAdapter.data[existingItemIndex] = item
    ...
    interface OnSnipListClickInterface{
        fun onClick(link: MovieUiKitModel)
    }
